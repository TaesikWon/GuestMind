...
class ResponseGenerator:
    ...
    def _base_response(self, text, emotion, cases):
        ...
        if cases and isinstance(cases, list):
            case_text = cases[0].get("text") if "text" in cases[0] else str(cases[0])
            summary = f"유사한 사례로 '{case_text}' 이(가) 있었습니다."
        else:
            ...
            messages=[
                {
                    ...
                    "content": (
                        "당신은 호텔 고객 경험 담당자입니다. "
                        "고객의 감정에 공감하며 따뜻하게 답변하세요. "
                        "마지막은 한 문장으로 위로하거나 감사 인사로 마무리하세요."
                    ),
                },
